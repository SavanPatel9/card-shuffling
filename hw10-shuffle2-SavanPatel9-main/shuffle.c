...
void divide(CardDeck origDeck, CardDeck * leftDeck, CardDeck * rightDeck)
{
  ...
  {
    leftDeck[i].size = i + 1;
    rightDeck[i].size = size - i;
    for(int j = 0; j < leftDeck[i].size; j++)
    {
      leftDeck[i].cards[j] = origDeck.cards[j];
    }
    for(int k = 0; k < rightDeck[i].size; k++)
    {
      rightDeck[i].cards[k] = origDeck.cards[k + i + 1];
    }
  }
}
#endif
...
// program.
static void interleaveHelper(CardDeck leftDeck, CardDeck rightDeck, CardDeck destDeck, int leftInd, int rightInd, int newInd, int round)
{
  if(leftInd == -1 || rightInd == -1)
  {
    if(leftInd != -1)
    {
      ...
      {
        destDeck.cards[i] = leftDeck.cards[i];

      }
    }
    if(rightInd != -1)
    {
      for(int i = rightInd; i >= 0; i--)
      {
        destDeck.cards[i] = rightDeck.cards[i];
      }
    }
  ...
  // pick one card from left deck
  destDeck.cards[newInd] = leftDeck.cards[leftInd];
  interleaveHelper(leftDeck, rightDeck, destDeck, leftInd - 1, rightInd, newInd - 1, round);

  // pick one card from right deck
  destDeck.cards[newInd] = rightDeck.cards[rightInd];
  interleaveHelper(leftDeck, rightDeck, destDeck, leftInd, rightInd - 1, newInd - 1, round);
...
void interleave(CardDeck leftDeck, CardDeck rightDeck, int round)
{
  ...
  int leftInd = leftDeck.size - 1;
  int rightInd = rightDeck.size - 1;
  int newInd = destDeck.size - 1;

  interleaveHelper(leftDeck, rightDeck, destDeck, leftInd, rightInd, newInd, round);
...
void shuffle(CardDeck origDeck, int round)
{
  ...
  CardDeck * left = malloc(sizeof(CardDeck) * numDecks);
  CardDeck * right = malloc(sizeof(CardDeck) * numDecks);

  divide(origDeck, left, right);

  for(int i = 0; i < numDecks; i++)
  {
    interleave(left[i], right[i], round);
  }
